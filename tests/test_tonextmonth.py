...
class TestTonextmonth(TestCase):
    ...
    def test_tonextmonth_transforms_datetime_into_the_time_of_next_month(self):
        d = SuperDateTime(dtnmth=datetime(2021, 10, 11, 17, 37, 27, 446259))
        self.assertEqual(d.dtnmth, datetime(2021,11, 1, 0, 0))

    def test_tonextmonth_transforms_date_into_the_time_of_next_month(self):
        d = SuperDateTime(dnmth=date(2021, 10, 11))
        self.assertEqual(d.dnmth, date(2021,11, 1))

    def test_tonextmonth_does_not_transform_if_is_not_datetime_or_date(self):
