...
def reference_key(field: JField) -> str:
    ...
    """
    if field.fdef._field_storage not in \
            [FieldStorage.FOREIGN_KEY, FieldStorage.LOCAL_KEY]:
        raise ValueError(f"field named {field.name} is not a reference field")
    if field.fdef._field_type == FieldType.LIST:
        return field.name + '_ids'
    elif field.fdef._field_type == FieldType.INSTANCE:
        return field.name + '_id'
    else:
        raise ValueError(f"field type {field.fdef._field_type} is not a "
